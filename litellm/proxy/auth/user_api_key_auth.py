...
def update_valid_token_with_end_user_params(
    ...
    # field in the request body, not on the API key). Always overwrite
    # them - including with `None` when the new end_user has no value -
    # so values from one end_user cannot leak onto a subsequent request
    # for a different end_user sharing the same virtual key. The previous
    # "only overwrite when non-None" policy was correct for *key*-level
    # fields (where custom auth may legitimately seed values that should
    # survive DB lookups returning None), but applying it to end-user
    # fields means stale values from a prior request's end_user survive
    # indefinitely on the cached `UserAPIKeyAuth` and override the
    # correct DB-resolved budget at enforcement time. See
    # `_copy_user_api_key_auth_for_cache` for the matching cache-side
    # strip.
    ...
    if end_user_id:
        ...
        try:
            ...
            verbose_proxy_logger.info(
                ...
                "model=%s zero_cost=%s end_user_skip_budget_checks=%s",
                route,
                end_user_id,
                user_api_key_auth_obj.api_key == LITELLM_PROXY_MASTER_KEY_ALIAS,
                _dbg_model,
